...
/**
 * @brief Check if s is a valid nickname
 *
 * Used for NICK command
 *
 * @param s The nickname to check
...
bool		AParser::isNickname( const std::string& s ) {
	...
	if (!(std::isalpha(*it) || AParser::isSpecial(*it))) //! Test first char
		return (false);
	for (it += 1; it != s.end(); it++){ //! Check for invalid char
		if (!isNickname(*it))
			return (false);
	}
	return (true);
...
std::string	AParser::getNickname(const char *buf, int &i, const int len) {
	...
	if (!(std::isalpha(buf[start]) || isSpecial(buf[start])))
		throw (AParser::InvalidNicknameException());
	i++;
...
const char*	AParser::InvalidNicknameException::what() const throw() {
